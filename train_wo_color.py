...
########## Dataset
parser.add_argument('--train_set', type=str, default='./data/our485', help='location of the train dataset')
...
def train():
    ...
    ValDataset = MemoryFriendlyLoader(img_dir=val_low_data_names, task='validation')
    ...
    for epoch in range(latest_epoch, args.epochs+1):
        ...
        # Train
        for inputs, labels in tqdm(train_queue, desc=f'Training Epoch {epoch}/{args.epochs}', unit='batch'):
            inputs = inputs.to(DEVICE)
            optimizer.zero_grad()

            loss = model._loss(inputs)
            loss.backward()
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(val_queue, desc=f'Validation Epoch {epoch}/{args.epochs}', unit='batch'):
                inputs = inputs.to(DEVICE)
                loss = model._loss(inputs)
