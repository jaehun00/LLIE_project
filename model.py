...
class Network(nn.Module):
    ...
    def __init__(self, stage=3):
        ...
        self.color_loss = nn.CosineSimilarity(dim=1, eps=1e-6)
    ...
    def _loss(self, input, target):
        ...
        for i in range(self.stage):
            ...
            losses += loss
        col_loss = 2 * torch.mean(1 -1 * self.color_loss(en_list[0], target))
        col_loss2 = torch.mean(1 -1 * self.color_loss(en_list[1], target))
        col_loss3 = torch.mean(1 -1 * self.color_loss(en_list[2], target))
        losses = losses + col_loss + col_loss2 + col_loss3
